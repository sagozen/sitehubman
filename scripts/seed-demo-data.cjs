/**
 * SITEHUB — Full Demo Data Seed Script (CJS compatible)
 *
 * Usage:
 *   node scripts/seed-demo-data.cjs seed    → seed all demo data
 *   node scripts/seed-demo-data.cjs reset   → clear + re-seed
 *   node scripts/seed-demo-data.cjs clear   → delete all demo data only
 */
...
const DEMO_PASSWORD = read('SITEHUB_DEMO_PASSWORD', 'demo1234');
const ADMIN_EMAIL = read('SITEHUB_ADMIN_EMAIL', 'admin@demo.com');
...
async function seedDemoData() {
  ...
  if (!signedIn) {
    try {
      ...
      console.warn(`  ⚠️ Warning: Could not sign in as admin@demo.com (${adminErr.message}).`);
    }
  }
  ...
  for (let i = 0; i < ORDERS.length; i++) {
    const t = ORDERS[i];
    const code = cardCode();
    const profileUrl = `https://sitehubman.com/c/${code}`;
...
async function main() {
  ...
  else console.log('Usage: node scripts/seed-demo-data.cjs [seed|clear|reset]');
}
