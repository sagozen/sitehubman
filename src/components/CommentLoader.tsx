...
type Props = {
  ...
  bubbleColor?: string;
  /** Number of bubbles to show together */
  count?: 1 | 2;
  style?: ViewStyle;
};
...
function LoaderBubble({ size, color, bubbleColor, delayOffset = 0, style }: BubbleProps) {
  ...
  useEffect(() => {
    startBounce(scale1, delayOffset);
    startBounce(scale2, delayOffset + STAGGER);
    startBounce(scale3, delayOffset + STAGGER * 2);
  }, [delayOffset, scale1, scale2, scale3, startBounce]);
 ...
 * Three dots bounce in sequence inside each rounded speech bubble.
 */
export function CommentLoader({
  ...
  const bubbleCount = count === 2 ? 2 : 1;

  if (bubbleCount === 1) {
    return <LoaderBubble size={size} color={color} bubbleColor={bubbleColor} style={style} />;
  }
  ...
  return (
    ...
    >
      <LoaderBubble size={size} color={color} bubbleColor={bubbleColor} delayOffset={0} />
      <LoaderBubble
        size={size * 0.86}
        color={color}
        bubbleColor={bubbleColor}
        delayOffset={STAGGER}
        style={{ marginLeft: -size * 0.18, marginTop: size * 0.16 }}
      />
    </View>
  );
}
