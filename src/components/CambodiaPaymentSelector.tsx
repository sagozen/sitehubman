  ...
  return (
    <View style={styles.list}>
      {methods.map((method) => {
        ...
        const bankImage = getPaymentMethodImage(method.id);
        ...
        return (
          ...
          >
            <View style={[styles.iconWrap, { backgroundColor: bankImage ? 'transparent' : `${accentColor}10` }]}>
              {bankImage ? (
                <Image source={bankImage} style={{ width: 36, height: 36 }} resizeMode="contain" />
              ) : (
                <PaymentMethodIcon
                  methodId={method.id}
