...
import { getOrder, saveNfcWrite, updateNfcStatus, updatePrinterJob, provisionNfcSecureKeys, getPasskeyForUrl } from '@/src/services/firestoreService';
import { type CardIdentity, getCardIdentity } from '@/src/services/cardIdentityService';
import { readNfcUrl, writeNfcUrl, writeAndLockNfc } from '@/src/services/nfcManagerService';
...
function SecureStepBar() {
  return (
    <View style={sb.row}>
      <View style={sb.item}>
        <View style={[sb.iconWrap, { backgroundColor: 'rgba(48,209,88,0.12)' }]}>
          <AppIcon name="CircleCheck" size={14} color={theme.status.success} />
        </View>
        <AppText style={[sb.label, { color: theme.statusText.success, fontWeight: '600' }]}>
          Print
        </AppText>
      </View>
      <View style={[sb.line, sb.lineDone]} />
      <View style={sb.item}>
        <View style={[sb.iconWrap, { backgroundColor: 'rgba(0,122,255,0.10)' }]}>
          <AppText style={{ color: theme.status.active, fontSize: 12, fontWeight: '700' }}>2</AppText>
        </View>
        <AppText style={[sb.label, { color: theme.statusText.active, fontWeight: '600' }]}>
          Encode
        </AppText>
      </View>
    </View>
...
export default function NfcProgrammingScreen() {
  ...
  const [provisionedPasskey, setProvisionedPasskey] = useState('');

  const payloadUrl = card
    ? card.lockedPublicProfileUrl || card.printProfileUrl || card.publicProfileUrl || provisionedUrl
    : provisionedUrl;
  ...
  useEffect(() => {
    if (payloadUrl && !editingUrl) {
      setEditingUrl(payloadUrl);
    }
  }, [payloadUrl]);
  ...
  async function handleUrlChange(text: string) {
    ...
    if (!card) return;
    try {
      const pass = await getPasskeyForUrl(text, card.cardId);
      setProvisionedPasskey(pass);
    } catch (err) {
      console.error('Failed to get passkey for URL', err);
    }
  }

  async function handleResetUrl() {
    if (!card) return;
    const defaultUrl = card.lockedPublicProfileUrl || card.printProfileUrl || card.publicProfileUrl || provisionedUrl;
    setEditingUrl(defaultUrl);
    try {
      const pass = await getPasskeyForUrl(defaultUrl, card.cardId);
      setProvisionedPasskey(pass);
    } catch (err) {
      console.error('Failed to get passkey for default URL', err);
    }
  }

  useEffect(() => {
    if (printed && !written && editingUrl && provisionedPasskey && quickMode && !loading && !provisioning) {
      const timer = setTimeout(() => {
        void handleWrite();
      }, 800);
      return () => clearTimeout(timer);
    }
  }, [printed, written, editingUrl, provisionedPasskey, quickMode, loading, provisioning]);
  ...
  async function handleWrite() {
    ...
    const urlToWrite = editingUrl.trim() || payloadUrl.trim();
    if (!urlToWrite) {
    ...
    try {
      ...
      await writeAndLockNfc(urlToWrite, provisionedPasskey || card.nfcPasskey || 'HPK-SECURE');
      await saveNfcWrite({
        chipUID: card.cardId,
        profileUrl: urlToWrite,
        orderId: order.id,
        cardId: card.cardId,
    ...
    if (quickMode) {
      setTimeout(async () => {
        try {
          ...
          console.error('Auto-print confirmation failed', err);
        }
      }, 3000);
    }
  }
  ...
  async function handleVerifyTap() {
    ...
    try {
      ...
      if (normalizeUrlForCompare(readUrl) !== normalizeUrlForCompare(urlToWrite)) {
        throw new Error(`NFC URL mismatch. Expected ${urlToWrite}, read ${readUrl}.`);
      }
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      <View style={styles.header}>
        <View style={styles.headerTop}>
              ...
              {printed && !written ? (
                <>
                  <AppText weight="semibold" style={styles.headerTitle}>
                    Job #{String(job?.queueNumber ?? '').slice(-4)} · Encode
                  </AppText>
                  <AppText variant="caption" tone="muted" style={styles.headerSub}>
                    {order ? `${order.customerName} · ${order.productType.replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase())}` : 'Loading...'}
                  </AppText>
                </>
              ) : (
                <>
          ...
          <Pressable 
            ...
            style={[styles.modeToggle, quickMode && styles.modeToggleActive]}
            hitSlop={8}
          >
            <AppIcon name={quickMode ? 'Zap' : 'Sliders'} size={12} color={quickMode ? '#007AFF' : theme.colors.textMuted} />
            <AppText style={[styles.modeToggleText, quickMode && styles.modeToggleTextActive]}>
              {quickMode ? 'Quick' : 'Manual'}
            </AppText>
          </Pressable>
        </View>
        {printed && !written ? <SecureStepBar /> : <StepBar current={currentStep} />}
      </View>
      ...
      <IosScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        {printed && !written ? (
          <>
            {/* Payload to write section */}
            <View style={styles.payloadSection}>
              <AppText variant="caption" weight="medium" style={styles.payloadLabel}>
                Payload to write
              </AppText>
              
              <View style={styles.darkPayloadBox}>
                <TextInput
                  style={styles.darkPayloadInput}
                  value={editingUrl}
                  ...
                  autoCorrect={false}
                />
                <Pressable onPress={handleResetUrl} style={styles.payloadRefreshBtn} hitSlop={8}>
                  <AppIcon name="RefreshCw" size={16} color="#FFF" />
                </Pressable>
              </View>
              
              <AppText variant="caption" tone="muted" style={styles.payloadSubLabel}>
                (Securely mapped via server)
              </AppText>

              {/* Passkey Badge */}
              <View style={styles.passkeyBadge}>
                <AppIcon name="Lock" size={14} color={theme.status.success} />
                <AppText style={styles.passkeyText}>
                  Unique Passkey:{' '}
                  <AppText weight="semibold" style={{ color: theme.colors.textPrimary }}>
                    {provisioning
                      ? '[SECURE_SERVER_GEN]'
                      : provisionedPasskey || card?.nfcPasskey || '[SECURE_SERVER_GEN]'}
                  </AppText>
                </AppText>
              </View>
              ...
              <View style={styles.amberNotice}>
                <AppText variant="caption" style={styles.amberNoticeText}>
                  locked - read-only after write with a unique, server-generated passkey
                </AppText>
              </View>
            </View>

            {/* NFC Tap Zone (matching screenshot) */}
            <View style={styles.secureTapZone}>
              <View style={styles.secureTapOrbit1}>
                <View style={styles.secureTapOrbit2}>
                  <View style={styles.secureTapOrb}>
                    <View style={styles.secureTapOrbInner} />
                  </View>
                </View>
              </View>
              <AppText weight="semibold" style={styles.secureTapTitle}>
                Tap blank NFC card
              </AppText>
              <AppText variant="caption" tone="muted" style={styles.secureTapSub}>
                Reader detecting...
              </AppText>
              
              {/* Progress bar */}
              <View style={styles.secureProgressBar}>
                <View style={[styles.secureProgressFill, { width: '65%' }]} />
              </View>
              
              {/* Loading spinner */}
              <View style={styles.spinnerWrap}>
                <Animated.View style={{ transform: [{ rotate: spin }] }}>
                  <AppIcon name="Loader2" size={16} color={theme.colors.textMuted} />
                </Animated.View>
              </View>
            </View>

            {/* Warning under tap zone */}
            <View style={{ alignItems: 'center', marginTop: 4 }}>
              <AppText variant="caption" style={{ color: '#FF3B30', fontWeight: '600' }}>
                Once locked, chip cannot be overwritten.
              </AppText>
            </View>
            ...
            <Pressable
              style={({ pressed }) => [
                styles.secureActionBtn,
                (loading || provisioning) && styles.secureActionBtnDisabled,
                pressed && !loading && !provisioning && styles.secureActionBtnPressed,
              ]}
              disabled={loading || provisioning}
              onPress={handleWrite}
            >
              <AppText style={styles.secureActionBtnText}>
                {provisioning
                  ? 'Write & Lock Card (Pending Secure Key...)'
                  : loading
                  ? 'Writing & Locking Tag...'
                  : 'Write & Lock Card'}
              </AppText>
...
const styles = StyleSheet.create({
  ...
  secureActionBtnDisabled: {
    backgroundColor: '#C7C7CC',
  },
  secureActionBtnPressed: {
    opacity: 0.8,
  },
  secureActionBtnText: {
    color: '#FFF',
