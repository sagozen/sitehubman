...
import { buildProductionQrPayload, needsSalesApproval } from '@/src/utils/orderProduction';
...
function DetailContent() {
  ...
  async function handleConfirmApproval(confirmation: SalesPaymentConfirmation) {
    ...
    setMessage(null);
    try {
      const approved = await confirmSalesProductionApproval(order.id, confirmation, user?.id);
      ...
      if (approved) {
        ...
        setMessage('Recorded as unpaid — not sent to printer.');
      }
      await load();
    } catch (error) {
    ...
    <SafeAreaView style={styles.safe}>
      ...
      <KeyboardAvoidingView style={styles.keyboard} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <IosScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
          ...
          {order.fulfillment === 'physical' || order.productType === 'physical_nfc' ? (
