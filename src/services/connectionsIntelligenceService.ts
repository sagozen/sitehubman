...
/**
 * Opens the native SMS / messaging app with a pre-filled message.
 */
export async function openSms(phone?: string, prefilledMessage?: string): Promise<'opened' | 'no_phone'> {
  const targetPhone = phone?.trim() || '+85512345678';
  const normalized = targetPhone.replace(/[^+\d]/g, '');
  if (!normalized) return 'no_phone';

  const message = prefilledMessage
    ? encodeURIComponent(prefilledMessage)
    : encodeURIComponent("Hi! We connected via SiteHub. Would love to stay in touch!");

  const smsUrl = Platform.OS === 'ios' ? `sms:${normalized}&body=${message}` : `sms:${normalized}?body=${message}`;
