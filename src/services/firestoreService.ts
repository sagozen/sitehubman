...
export async function provisionNfcSecureKeys(
  cardId: string,
  jobId: string,
  customerName: string
): Promise<{ url: string; nfcPasskey: string }> {
  assertNonEmpty(cardId, 'Card ID is required.');
  
  const cardRef = doc(db, firebaseCollections.cards, cardId);
  const cardSnap = await getDoc(cardRef);
  if (!cardSnap.exists()) {
    throw new Error(`Card ${cardId} not found.`);
  }
  
  const data = cardSnap.data() as Record<string, any>;
  let lockedUrl = data.lockedPublicProfileUrl || data.printProfileUrl || data.publicProfileUrl;
  let passkey = data.nfcPasskey;
  ...
  if (!lockedUrl?.trim()) {
    const initials = customerName
      ? customerName.split(/\s+/).map(part => part[0]).join('').toUpperCase().substring(0, 2)
      : 'SH';
    const cleanJobId = jobId.slice(-4);
    const suffix = Math.random().toString(36).substring(2, 6).toUpperCase();
    lockedUrl = `sitehub.bio/c/${initials}-${cleanJobId}-${suffix}`;
    updatePayload.lockedPublicProfileUrl = lockedUrl;
    updatePayload.printProfileUrl = lockedUrl;
    updatePayload.publicSlug = `${initials}-${cleanJobId}-${suffix}`;
    updated = true;
  }
  
  if (!passkey?.trim()) {
    passkey = `HASHED-PASSKEY-${Math.random().toString(36).substring(2, 6).toUpperCase()}`;
    updatePayload.nfcPasskey = passkey;
    updated = true;
  }
...
export async function getPasskeyForUrl(url: string, cardId: string): Promise<string> {
  assertNonEmpty(cardId, 'Card ID is required.');
  ...
  if (cardSnap.exists()) {
    ...
    if (data.nfcPasskey && (data.lockedPublicProfileUrl === url || data.publicProfileUrl === url || data.printProfileUrl === url)) {
      return data.nfcPasskey;
    }
  }
