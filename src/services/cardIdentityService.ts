...
export async function lockCardForProduction(cardId: string, orderId: string, userId: string): Promise<CardIdentity> {
  ...
  if (!card) throw new Error('Card not found. Create the card before approving production.');
  if (!card.publicProfileUrl.trim()) {
    throw new Error('Card public profile URL is missing. Cannot approve production.');
  }
