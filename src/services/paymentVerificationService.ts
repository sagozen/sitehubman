...
/**
 * Simplified flow helper — an order is "sales approved" when it has
 * salesApprovedAt set regardless of which payment status was used.
 */
export function isSalesApproved(order: Pick<Order, 'salesApprovedAt'> | null | undefined): boolean {
  return Boolean(order?.salesApprovedAt);
}
