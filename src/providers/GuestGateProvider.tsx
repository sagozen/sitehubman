...
import { AuthSignupSheet } from '../features/auth/components/AuthSignupSheet';
      ...
      <AuthSignupSheet
        ...
        onSuccess={async (user) => {
          try {
            ...
            if (!stayedOnScreen) {
              let redirectTarget: any = '/(tabs)';
              try {
                redirectTarget = await getPostAuthDestination(user, { intent: gate.unlockIntent });
              } catch (err) {
                console.error('[GuestGateProvider] getPostAuthDestination failed:', err);
              }
              ...
              try {
                ...
                console.error('[GuestGateProvider] migration verification failed:', err);
              }

              router.replace(redirectTarget);
            }
          } catch (err) {
            console.error('[GuestGateProvider] onSuccess redirect error:', err);
            router.replace('/(tabs)');
          } finally {
            gate.closeUnlock();
          }
